...
class AgentManager:
    def __init__(self, task_desc: str, cfg: Any, workspace_dir: Path):
        ...
        # Extract end_stage from config
        self.end_stage = getattr(cfg, 'end_stage', 4)
        print(f"[cyan]AgentManager configured to run stages 1-{self.end_stage}[/cyan]")
...
"""
    ...
    def _create_next_main_stage(
        ...
        if main_stage_num >= self.end_stage:
            print(f"[green]Reached end_stage {self.end_stage}, stopping stage creation[/green]")
            return None
