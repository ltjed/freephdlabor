...
class SerperAPI(SearchAPI):
    def __init__(
        self, api_key: Optional[str] = None, config: Optional[SerperConfig] = None
    ):
        if api_key:
    ...
    def extract_fields(
        ...
        return [
            {key: item.get(key, "") for key in fields if key in item} for item in items
        ]
        ...
        try:
            ...
            results = {
                "organic": self.extract_fields(
                    data.get("organic", []), ["title", "link", "snippet", "date"]
                ),
                "topStories": self.extract_fields(
                    data.get("topStories", []), ["title", "imageUrl"]
                ),
                "images": self.extract_fields(
                    data.get("images", [])[:6], ["title", "imageUrl"]
                ),
...
def create_search_api(
    ...
    if search_provider.lower() == "serper":
        ...
        raise ValueError(
            f"Invalid search provider: {search_provider}. Must be 'serper' or 'searxng'"
        )
